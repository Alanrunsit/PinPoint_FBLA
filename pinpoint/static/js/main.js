...
const PinPoint = (() => {
        ...
        return `
            <div class="business-card" onclick="PinPoint.goToBusiness(${biz.id}, event)">
                ...
                <div class="card-body">
                    ...
                    ${biz.website_url ? '<a class="card-website" href="' + biz.website_url + '" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation();">\uD83C\uDF10 Visit Website</a>' : ''}
                </div>
            </div>
        `;
    }
    ...
    async function initBusinessPage() {
        ...
        try {
            ...
            if (biz.website_url) {
                websiteEl.href = biz.website_url;
                websiteEl.textContent = '\uD83C\uDF10 ' + biz.website_url.replace(/^https?:\/\/(www\.)?/, '').replace(/\/$/, '');
